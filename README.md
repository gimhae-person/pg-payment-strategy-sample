...
KCP, INICIS처럼 PG사별 결제 요청 포맷과 승인 API가 다른 환경에서, 실무에서 다뤘던 결제 흐름을 포트폴리오용으로 일반화해 재구성한 Java 샘플입니다.

회사 자산이나 민감한 비즈니스 로직이 드러나지 않도록 구체 구현은 단순화했고, 웹훅 반영 흐름을 포함한 일부 샘플 정리는 AI-assisted coding을 보조적으로 활용했습니다. 다만 결제 흐름에 대한 문제 정의, 공통/개별 책임 분리, 구조 설계 방향은 실제 구현 경험을 바탕으로 정리했습니다.
...
실무 결제 연동에서 KCP/INICIS PG별 승인 파라미터와 서버 간 검증 흐름이 달랐기 때문에, 결제 준비/클라이언트 승인값 수신/백엔드 승인 API 호출/webhook 정합성 처리 흐름을 전략 패턴 기반의 PG 어댑터 구조로 재구성했습니다. 결제 유스케이스는 공통 서비스에서 처리하고, PG별 요청 포맷과 서명 및 승인 로직은 각 어댑터 내부에 격리해 PG 추가나 교체 시 변경 범위를 최소화했습니다. 이 저장소는 실제 업무 경험을 바탕으로 개인 포트폴리오용으로 일반화한 샘플입니다.
